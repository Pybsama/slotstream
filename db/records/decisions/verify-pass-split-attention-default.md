...
**Evidence.** Paired in one process at k=3, the split minus the dense kernel measured +6.1 to +6.8 ms at 4,068 tokens, -1.7 at 6,116, -3.9 at 8,183, -7.8 at 12,279 and -11.2 at 16,356; the fetch-free pass is 32% cheaper at 32,740 tokens and 45% at 65,508. On one warm engine at the 22 GB default profile with a 16,356-token prompt, a verification round took 12% less time with the split in two runs, and on a quiet machine after the commit speculative decode ran at 11.82 against 11.67 tok/s (x1.013) there and x1.29 with a 32,740-token prompt. The 16k gain is small because the split accepts fewer drafts on that prompt, a rounding artifact that can fall either way; at 32k both arms accept alike, so the ratio is the per-pass saving. The split changes speed, not the guarantee: like the dense pass, its rows can round differently from one-row decode, and acceptance samples from the pass's own logits either way. Record: [[records/measurements/speculative-verify-pass-split-attention-2026-09-17]].
