...
updated: 2026-09-14T14:01:46.378812+00:00
...
needle: Shortest conversation written to `--prefix-cache-dir` (default 2048 tokens)
...
status: current
---
The code-defined opt-in default for `serve --prefix-cache-min-tokens` is 2048 tokens. `PersistentPrefixConfiguration.defaultMinimumTokens` owns the value and the CLI help interpolates it; `persistent-prefix-policy` pins it. Every write stores the fixed recurrent arrays, and a conversation's first write also stores every cached row, so shorter conversations are not written by default. This is a provisional engineering choice, not a measured break-even: [[records/design/measured-operating-policies]] states its tradeoff and revision criterion, and [[records/measurements/persistent-prefix-cache-2026-09-14]] records measured save and restore costs at 2051, about 3,900 and about 15,700 tokens.
