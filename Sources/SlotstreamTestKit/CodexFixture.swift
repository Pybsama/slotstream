// A real Codex 0.148.0 first-turn request, captured on 2026-09-16 against the
// stub Responses server in the session scratchpad (`responses_stub.py`), with
// the long prose fields trimmed and per-run identifiers blanked. Every tool
// schema and every item shape is verbatim: this is what a custom provider
// receives from `codex exec` with an unknown model slug.
...
extension Catalogue {
    static let codexTurnOneFixture = #"""
{
 "model": "qwen3.8-flash-next:4bit",
 "instructions": "You are a coding agent running in the Codex CLI, a terminal-based coding assistant. Codex CLI is an open source project led by OpenAI. You are expected to be pr [trimmed]",
 "input": [
  {
   "type": "message",
   "id": "msg_fixture",
   ...
   "content": [
    {
     "type": "input_text",
     "text": "<skills_instructions>\n## Skills\nA skill is a set of instructions provided through a `SKILL.md` source. Below is the list of skills that can be used. Each entry includes a name, description, and source [trimmed]"
    },
    {
     "type": "input_text",
  ...
  },
  {
   ...
   "content": [
    {
     "type": "input_text",
     "text": "<environment_context>\n  <cwd>/private/tmp/claude-502/-Users-carlos-Projects-command-center/42731ad3-8883-4fdb-b4ff-f4ded98c97d2/scratchpad/repo</cwd>\n  <shell>zsh</shell>\n  <current_date>2026-09-16</c [trimmed]"
    }
   ]
  },
  {
...
"""#
    ...
    static func responsesCodexFixture() -> CheckReport {
        var c = CheckBuilder("responses-codex-fixture")
        guard let data = codexTurnOneFixture.data(using: .utf8),
            let body = (try? JSONSerialization.jsonObject(with: data)) as? [String: Any]
        else {
            c.expect("fixture is JSON", false)
            return c.report()
        }
        do {
            let r = try ResponsesDialect.parse(body)
            c.equal("every tool Codex sends is rendered or dropped on purpose", r.tools.map(\.name), [
            ...
            c.equal("the namespace is remembered", r.namespaces, ["multi_agent_v1"])
            c.equal("developer context joins the instructions; two user turns follow", r.messages.map(\.role), ["system", "user", "user"])
            c.expect("system starts with the base instructions", r.messages[0].content.hasPrefix("You are a coding agent"))
            c.expect("...and carries the permissions block", r.messages[0].content.contains("<permissions instructions>"))
            c.expect("environment context is a user turn", r.messages[1].content.hasPrefix("<environment_context>"))
            c.equal("the prompt is the last user turn", r.messages[2].content, "Say hello")
            c.expect("no effort means no thinking", !r.thinking && r.effort == nil)
            c.expect("Codex streams", r.stream)
            c.expect("parallel calls allowed", r.parallel)
            c.expect("auto choice", r.choice == .auto)
            c.expect("no output limit is sent", r.maxOutputTokens == nil)
            c.equal("exec_command prefix_rule is typed as an array", r.tools[0].schema.params["prefix_rule"], .array)
            c.equal("exec_command yield_time_ms is a number", r.tools[0].schema.params["yield_time_ms"], .number)
            c.equal("request_user_input questions is an array", r.tools[3].schema.params["questions"], .array)
        } catch { c.expect("Codex's real first request parses", false, "\(error)") }
        return c.report()
    }
}
