...
func realBasicsCheckIfRequested() async throws -> Bool {
    ...
    var receipt: [String: Any] = ["model_plan_gb": 10]
    ...
    let ask = try await runtime!.newThread(title: "PDF question")
    ...
    receipt["pdf"] = ["prompt": question, "state": thread.run?.state.rawValue ?? "", "answer": answer, "trace": thread.run?.trace ?? [],
                      "excerpts": excerpts.map { ["id": $0.id, "path": $0.path, "page": $0.page ?? 0, "method": $0.method ?? "", "sha256": $0.hash] as [String: Any] }]
    ...
    let edit = try await runtime!.newThread(title: "Plan edit")
    ...
    receipt["edit"] = ["prompt": request, "state": thread.run?.state.rawValue ?? "", "trace": thread.run?.trace ?? [], "status": thread.run?.status ?? "",
                       "changes": (set?.changes ?? []).map { ["path": $0.display, "operation": $0.operation.rawValue, "added": $0.added, "removed": $0.removed] as [String: Any] }]
    ...
    let build = try await runtime!.newThread(title: "Counter app")
    ...
    receipt["app"] = ["prompt": appRequest, "state": thread.run?.state.rawValue ?? "", "status": thread.run?.status ?? "", "trace": thread.run?.trace ?? [],
                      ...
                      "html_bytes": proposal?.html.utf8.count ?? 0, "html_sha256": proposal.map { digestText($0.html) } ?? "", "notes": proposal?.notes ?? []]
